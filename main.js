	...
	function (flags, url, key) {
		return _Utils_Tuple2(
			$author$project$Model$Model(
				...
				A3($author$project$Model$MouseState, 0, 0, $author$project$Model$NoneSelected))($elm$core$Maybe$Nothing)(0),
...
var $author$project$ViewVariables$blockHeight = ($author$project$ViewVariables$blockWidth / 4) | 0;
...
var $author$project$ViewPositions$countOutputsBefore = F2(
		...
		while (true) {
			if (!threshhold) {
				...
				if (!inputs.b) {
					...
					if (input.$ === 'Output') {
						var id = input.a;
						return 1 + A2($author$project$ViewPositions$countOutputsBefore, rest, threshhold - 1);
					} else {
...
var $author$project$ViewPositions$countOutputs = function (inputs) {
	return A2(
		$author$project$ViewPositions$countOutputsBefore,
		inputs,
...
var $author$project$ViewVariables$viewportWidth = 1000;
var $author$project$ViewPositions$getViewportHeight = F2(
	function (windowWidth, windowHeight) {
		return (($author$project$ViewVariables$viewportWidth * windowHeight) / windowWidth) | 0;
	});
...
var $author$project$ViewPositions$mouseToSvgCoordinates = F5(
		...
		while (true) {
			if (!func.b) {
				...
				if (_Utils_eq(call.id, id)) {
					...
					id = $temp$id;
					func = $temp$func;
					continue getCallById;
				}
			}
		}
	});
var $author$project$Update$finishBlockAtPos = F2(
	function (func, blockId) {
		if (!func.b) {
			return _List_Nil;
		} else {
			var currentCall = func.a;
			var calls = func.b;
			return _Utils_eq(currentCall.id, blockId) ? calls : A2(
				$elm$core$List$cons,
				currentCall,
				A2($author$project$Update$finishBlockAtPos, calls, blockId));
		}
	});
var $author$project$Update$placeBlockAtPos = F5(
	function (func, blockId, blockPos, blockPositions, call) {
		placeBlockAtPos:
		while (true) {
			if (!func.b) {
				return _List_fromArray(
				...
				var currentCall = func.a;
				var calls = func.b;
				if (_Utils_eq(currentCall.id, blockId)) {
					var $temp$func = calls,
						$temp$blockId = blockId,
						$temp$blockPos = blockPos,
						$temp$blockPositions = blockPositions,
						$temp$call = call;
					func = $temp$func;
					blockId = $temp$blockId;
					blockPos = $temp$blockPos;
					blockPositions = $temp$blockPositions;
					call = $temp$call;
					continue placeBlockAtPos;
				} else {
					var _v1 = A2($elm$core$Dict$get, currentCall.id, blockPositions);
					if (_v1.$ === 'Nothing') {
						...
						return (_Utils_cmp(blockPos.ypos, currentBlockPos.ypos) < 0) ? A2(
							...
							$elm$core$List$cons,
							currentCall,
							A5($author$project$Update$placeBlockAtPos, calls, blockId, blockPos, blockPositions, call));
					}
				}
			}
		}
...
var $author$project$ViewVariables$programHeight = function (windowHeight) {
	return (windowHeight - $author$project$ViewVariables$svgYpos) - $author$project$ViewVariables$scrollbarWidth;
};
var $author$project$ViewVariables$programWidth = function (windowWidth) {
	return windowWidth - (2 * $author$project$ViewVariables$scrollbarWidth);
};
var $author$project$ViewVariables$toolbarWidth = function (windowWidth) {
	return ($author$project$ViewVariables$viewportWidth / 4) | 0;
};
var $author$project$Update$funcBlockDropped = F5(
	function (func, blockId, oldMouse, windowWidth, windowHeight) {
		var blockPositions = A6(
			$author$project$ViewPositions$getBlockPositions,
			func,
			oldMouse,
			$author$project$ViewVariables$programWidth(windowWidth),
			$author$project$ViewVariables$programHeight(windowHeight),
			$author$project$ViewVariables$functionXSpacing + $author$project$ViewVariables$toolbarWidth(windowWidth),
			0);
		var _v0 = A2($elm$core$Dict$get, blockId, blockPositions);
		if (_v0.$ === 'Nothing') {
			return A2($elm$core$Debug$log, 'No block in funcBlockDropped', func);
		} else {
			var blockPos = _v0.a;
			var _v1 = A2($author$project$Model$getCallById, blockId, func);
			if (_v1.$ === 'Nothing') {
		...
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					mouseState: newMouse,
					program: A5(
						$author$project$Update$programBlockDropped,
						model.program,
						id,
						oldMouse,
						$author$project$ViewVariables$programWidth(model.windowWidth),
						$author$project$ViewVariables$programHeight(model.windowHeight))
				}),
			$elm$core$Platform$Cmd$none);
	});
...
var $author$project$Update$nodeOutputId = function (callid) {
	...
	function (model, id) {
		...
		return _Utils_Tuple2(
			...
			A2(
				...
				$author$project$Model$SilentDomError,
				$elm$browser$Browser$Dom$focus(
					$author$project$Update$nodeOutputId(id))));
	});
...
var $author$project$Update$spawnBlockFunc = F2(
	function (func, call) {
		return A2($elm$core$List$cons, call, func);
	});
var $author$project$Update$spawnBlockProgram = F2(
	function (onion, call) {
		if (!onion.b) {
			return _List_fromArray(
				[
			...
			return A2(
				$elm$core$List$cons,
				A2($author$project$Update$spawnBlockFunc, func, call),
				funcs);
		}
	});
		...
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					...
					mouseState: newMouse,
					program: A2($author$project$Update$spawnBlockProgram, model.program, newCall)
				}),
			$elm$core$Platform$Cmd$none);
	});
	...
	function (msg, model) {
		switch (msg.$) {
			...
			case 'InputClick':
				var id = msg.a;
				var index = msg.b;
				var l = A2($elm$core$Debug$log, 'left', 2);
			...
			case 'InputRightClick':
				var id = msg.a;
				var index = msg.b;
				var m = A2($elm$core$Debug$log, 'r', 2);
...
var $author$project$View$listing = F3(
...
var $author$project$View$makePage = F3(
...
var $author$project$ViewPositions$createViewboxDimensions = F2(
	function (w, h) {
		...
		var height = $elm$core$String$fromInt(h);
		return width + (' ' + height);
	});
...
var $author$project$SvgDraw$functionNameshape = F2(
					...
					[
						...
						_List_fromArray(
							[
								...
								$elm$core$String$fromInt($author$project$ViewVariables$blockWidth)),
						...
						_List_Nil),
						A5($author$project$SvgDraw$svgText, ($author$project$ViewVariables$blockWidth / 2) | 0, ($author$project$ViewVariables$blockHeight / 2) | 0, call.functionName, $author$project$ViewVariables$funcNameFontHeight, 'white')
					]));
		} else {
...
var $author$project$SvgDraw$drawBuiltIn = F3(
	function (call, index, viewStructure) {
		...
		if (get.$ === 'Just') {
			var names = get.a;
			return A2($author$project$SvgDraw$functionNameshape, call, viewStructure);
		} else {
			return $author$project$SvgDraw$errorSvgNode('not a built in function ' + call.functionName);
		}
	});
	...
	function (func, viewStructure, counter) {
		if (!func.b) {
			...
			return A2(
				...
				A3($author$project$DrawFunc$drawFunc, calls, viewStructure, counter + 1));
		}
	});
...
var $author$project$DrawFunc$drawFuncEndings = F3(
	function (func, blockPositions, mouseState) {
		if (!func.b) {
			...
			return A2(
				$elm$core$List$cons,
				A3($author$project$DrawFunc$drawCallEnding, call, blockPositions, mouseState),
				A3($author$project$DrawFunc$drawFuncEndings, calls, blockPositions, mouseState));
		}
	});
...
var $author$project$SvgDraw$taxiLine = F3(
	function (posList, inputEvent, isLineHighlighted) {
...
var $author$project$SvgDraw$drawConnector = F7(
	function (call, blockPos, inputCounter, otherBlockPos, inputEvent, isLineHighlighted, routing) {
		if (routing.$ === 'Nothing') {
...
var $author$project$DrawFunc$drawOutputLine = F8(
	function (call, blockPos, inputCounter, viewStructure, inputEvent, isLineHighlighted, routing, outputId) {
		var _v0 = A2($elm$core$Dict$get, outputId, viewStructure.blockPositions);
		if (_v0.$ === 'Nothing') {
			return $author$project$SvgDraw$errorSvgNode('Can\'t find line output');
		} else {
			var otherBlockPos = _v0.a;
			return A7($author$project$SvgDraw$drawConnector, call, blockPos, inputCounter, otherBlockPos, inputEvent, isLineHighlighted, routing);
		}
	});
...
var $author$project$DrawFunc$drawInput = F6(
	function (call, input, blockPos, inputCounter, viewStructure, routing) {
		...
		var isInputHighlighted = function () {
			var _v3 = viewStructure.mouseState.mouseSelection;
		...
		switch (input.$) {
			case 'Output':
				...
				return A3(
					...
					_List_fromArray(
						[
							A8($author$project$DrawFunc$drawOutputLine, call, blockPos, inputCounter, viewStructure, outputEvent, isLineHighlighted, routing, id),
...
var $author$project$DrawFunc$drawInputLines = F6(
	function (call, inputs, blockPos, inputCounter, viewStructure, lineRouting) {
		if (!inputs.b) {
			return _List_fromArray(
				[
					A4(
			...
			if (!lineRouting.b) {
				return _List_fromArray(
					[
						$author$project$SvgDraw$errorSvgNode('not enough routings for call')
					]);
			} else {
				var routing = lineRouting.a;
				var restRouting = lineRouting.b;
				return A2(
					$elm$core$List$cons,
					A6($author$project$DrawFunc$drawInput, call, input, blockPos, inputCounter, viewStructure, routing),
					A6($author$project$DrawFunc$drawInputLines, call, rest, blockPos, inputCounter + 1, viewStructure, restRouting));
			}
		}
	});
var $author$project$DrawFunc$drawCallInputs = F4(
	function (call, viewStructure, mouseState, routingList) {
		var _v0 = A2($elm$core$Dict$get, call.id, viewStructure.blockPositions);
		if (_v0.$ === 'Just') {
			var blockPos = _v0.a;
			return A2(
				$elm$svg$Svg$g,
				_List_Nil,
				A6($author$project$DrawFunc$drawInputLines, call, call.inputs, blockPos, 0, viewStructure, routingList));
		} else {
			return $author$project$SvgDraw$errorSvgNode('Call without a block position');
		}
	});
var $author$project$DrawFunc$drawFuncInputs = F4(
	function (func, viewStructure, mouseState, lineRouting) {
		if (!func.b) {
			return _List_Nil;
		} else {
			var call = func.a;
			...
			if (!lineRouting.b) {
				return _List_fromArray(
					[
						$author$project$SvgDraw$errorSvgNode('lineRouting not big enough')
					]);
			} else {
				var routing = lineRouting.a;
				var restRouting = lineRouting.b;
				return A2(
					$elm$core$List$cons,
					A4($author$project$DrawFunc$drawCallInputs, call, viewStructure, mouseState, routing),
					A4($author$project$DrawFunc$drawFuncInputs, calls, viewStructure, mouseState, restRouting));
			}
		}
	});
var $author$project$DrawFunc$drawFuncWithConnections = F2(
	function (viewStructure, mouseState) {
		return A2(
			...
			_List_fromArray(
				[
					A2($author$project$SvgDraw$svgTranslate, viewStructure.funcxoffset, viewStructure.funcyoffset)
				]),
			_List_fromArray(
				[
					A2(
					...
					$elm$svg$Svg$g,
					_List_Nil,
					A4($author$project$DrawFunc$drawFuncInputs, viewStructure.sortedFunc, viewStructure, mouseState, viewStructure.lineRouting)),
					A2(
					$elm$svg$Svg$g,
					_List_Nil,
					A3($author$project$DrawFunc$drawFuncEndings, viewStructure.sortedFunc, viewStructure.blockPositions, mouseState))
				]));
	});
var $author$project$ViewPositions$ViewStructure = F7(
	function (blockPositions, lineRouting, sortedFunc, funcxoffset, funcyoffset, mouseState, isToolbar) {
		return {blockPositions: blockPositions, funcxoffset: funcxoffset, funcyoffset: funcyoffset, isToolbar: isToolbar, lineRouting: lineRouting, mouseState: mouseState, sortedFunc: sortedFunc};
	});
...
var $author$project$ViewPositions$countOutputsBetween = F3(
	function (subConnectedArray, startIndex, endIndex) {
		if (_Utils_cmp(endIndex, startIndex + 1) < 1) {
			return 0;
		} else {
			var _v0 = A2($elm$core$Array$get, startIndex + 1, subConnectedArray);
			if (_v0.$ === 'Nothing') {
				return 0;
			} else {
				var connectedness = _v0.a;
				return connectedness + A3($author$project$ViewPositions$countOutputsBetween, subConnectedArray, startIndex + 1, endIndex);
			}
		}
	});
var $author$project$ViewPositions$getOutputRouting = F5(
	function (id, connectedArray, idToPos, isLeft, thisCallId) {
		var _v0 = A2($elm$core$Dict$get, id, idToPos);
		if (_v0.$ === 'Nothing') {
			return 0;
		} else {
			var outputIndex = _v0.a;
			var _v1 = A2($elm$core$Dict$get, thisCallId, idToPos);
			if (_v1.$ === 'Nothing') {
				return 0;
			} else {
				var callIndex = _v1.a;
				if (_Utils_eq(outputIndex, callIndex - 1)) {
					return 0;
				} else {
					var subConnectedArray = isLeft ? connectedArray.a : connectedArray.b;
					var startingSign = isLeft ? (-1) : 1;
					return startingSign + (startingSign * A3($author$project$ViewPositions$countOutputsBetween, subConnectedArray, outputIndex, callIndex));
				}
			}
		}
	});
var $author$project$ViewPositions$getInputsRouting = F5(
	function (inputs, connectedArray, idToPos, isLeft, thisCallId) {
		if (!inputs.b) {
			return _Utils_Tuple2(_List_Nil, isLeft);
		} else {
			var input = inputs.a;
			var rest = inputs.b;
			if (input.$ === 'Output') {
				var id = input.a;
				var restAnswer = A5($author$project$ViewPositions$getInputsRouting, rest, connectedArray, idToPos, !isLeft, thisCallId);
				return _Utils_Tuple2(
					A2(
						$elm$core$List$cons,
						$elm$core$Maybe$Just(
							A5($author$project$ViewPositions$getOutputRouting, id, connectedArray, idToPos, isLeft, thisCallId)),
						restAnswer.a),
					restAnswer.b);
			} else {
				var restAnswer = A5($author$project$ViewPositions$getInputsRouting, rest, connectedArray, idToPos, isLeft, thisCallId);
				return _Utils_Tuple2(
					A2($elm$core$List$cons, $elm$core$Maybe$Nothing, restAnswer.a),
					restAnswer.b);
			}
		}
	});
var $author$project$ViewPositions$getLineRouting = F4(
	function (func, connectedArray, idToPos, isLeft) {
		if (!func.b) {
			return _List_Nil;
		} else {
			var call = func.a;
			var calls = func.b;
			var routing = A5($author$project$ViewPositions$getInputsRouting, call.inputs, connectedArray, idToPos, isLeft, call.id);
			return A2(
				$elm$core$List$cons,
				routing.a,
				A4($author$project$ViewPositions$getLineRouting, calls, connectedArray, idToPos, routing.b));
		}
	});
...
var $author$project$ViewPositions$updateConnectedArray = F4(
	function (inputs, indexToPos, array, isLeft) {
		updateConnectedArray:
		while (true) {
			if (!inputs.b) {
				return _Utils_Tuple2(isLeft, array);
			} else {
				var input = inputs.a;
				var rest = inputs.b;
				if (input.$ === 'Output') {
					var id = input.a;
					var _v2 = A2($elm$core$Dict$get, id, indexToPos);
					if (_v2.$ === 'Nothing') {
						var $temp$inputs = rest,
							$temp$indexToPos = indexToPos,
							$temp$array = array,
							$temp$isLeft = isLeft;
						inputs = $temp$inputs;
						indexToPos = $temp$indexToPos;
						array = $temp$array;
						isLeft = $temp$isLeft;
						continue updateConnectedArray;
					} else {
						var pos = _v2.a;
						if (isLeft) {
							var $temp$inputs = rest,
								$temp$indexToPos = indexToPos,
								$temp$array = _Utils_Tuple2(
								A3($elm$core$Array$set, pos, 1, array.a),
								array.b),
								$temp$isLeft = false;
							inputs = $temp$inputs;
							indexToPos = $temp$indexToPos;
							...
							var $temp$inputs = rest,
								...
								$temp$array = _Utils_Tuple2(
								array.a,
								A3($elm$core$Array$set, pos, 1, array.b)),
								$temp$isLeft = true;
							inputs = $temp$inputs;
							indexToPos = $temp$indexToPos;
							array = $temp$array;
...
var $author$project$ViewPositions$getOutputConnectedArrayHelper = F4(
	function (func, indexToPos, array, isLeft) {
		getOutputConnectedArrayHelper:
		while (true) {
			if (!func.b) {
				return array;
			} else {
				var call = func.a;
				var calls = func.b;
				var update = A4($author$project$ViewPositions$updateConnectedArray, call.inputs, indexToPos, array, isLeft);
				var $temp$func = calls,
					$temp$indexToPos = indexToPos,
					$temp$array = update.b,
					$temp$isLeft = update.a;
				func = $temp$func;
				indexToPos = $temp$indexToPos;
				array = $temp$array;
				isLeft = $temp$isLeft;
				continue getOutputConnectedArrayHelper;
			}
		}
	});
...
var $author$project$ViewPositions$getOutputConnectedArray = F2(
	function (func, indexToPos) {
		var oneArray = A2(
			$elm$core$Array$repeat,
			$elm$core$List$length(func),
			0);
		return A4(
			$author$project$ViewPositions$getOutputConnectedArrayHelper,
			func,
			indexToPos,
			_Utils_Tuple2(oneArray, oneArray),
			true);
	});
	...
	function (func, dict, currentPos) {
		if (!func.b) {
			...
			return A3(
				...
				call.id,
				currentPos,
				A3($author$project$ViewPositions$idToPosAdd, calls, dict, currentPos + 1));
		}
	});
var $elm$core$List$sortBy = _List_sortBy;
var $author$project$ViewPositions$makeIdToPos = F2(
	function (func, blockPositions) {
		var sorted = A2(
			$elm$core$List$sortBy,
			$author$project$ViewPositions$blockSorter(blockPositions),
			func);
		return _Utils_Tuple2(
			sorted,
			A3($author$project$ViewPositions$idToPosAdd, sorted, $elm$core$Dict$empty, 0));
	});
var $author$project$ViewPositions$getViewStructure = F7(
	function (func, mouseState, svgScreenWidth, svgScreenHeight, xoffset, yoffset, isToolbar) {
		...
		var madePos = A2($author$project$ViewPositions$makeIdToPos, func, blockPositions);
		var idToPos = madePos.b;
		var sortedFunc = madePos.a;
		var connectedArray = A2($author$project$ViewPositions$getOutputConnectedArray, sortedFunc, idToPos);
		var lineRouting = A4($author$project$ViewPositions$getLineRouting, sortedFunc, connectedArray, idToPos, true);
		return A7($author$project$ViewPositions$ViewStructure, blockPositions, lineRouting, sortedFunc, xoffset, yoffset, mouseState, isToolbar);
	});
	...
	function (onion, mouseState, svgWindowWidth, svgWindowHeight) {
		if (!onion.b) {
			...
			var viewStructure = A7(
				...
				svgWindowWidth,
				svgWindowHeight,
				$author$project$ViewVariables$functionXSpacing + $author$project$ViewVariables$toolbarWidth(svgWindowWidth),
				0,
				false);
			return A2(
				$elm$core$List$cons,
				A2($author$project$DrawFunc$drawFuncWithConnections, viewStructure, mouseState),
				A4($author$project$DrawProgram$drawOnion, funcs, mouseState, svgWindowWidth, svgWindowHeight));
		}
	});
...
var $author$project$DrawToolbar$drawToolbar = F4(
	function (onion, mouseState, svgWindowWidth, svgWindowHeight) {
		return A2(
			$author$project$DrawFunc$drawFuncWithConnections,
			A7($author$project$ViewPositions$getViewStructure, $author$project$BuiltIn$allBuiltInAsFunction, mouseState, svgWindowWidth, svgWindowHeight, 0, 0, true),
			mouseState);
	});
...
var $author$project$DrawProgram$drawProgram = F4(
	function (program, mouseState, svgWindowWidth, svgWindowHeight) {
		var elementHeight = svgWindowHeight * 2;
		var viewportHeight = A2($author$project$ViewPositions$getViewportHeight, svgWindowWidth, elementHeight);
			...
			A2(
				...
				_List_fromArray(
					[
						...
						$elm$svg$Svg$Attributes$height(
						$elm$core$String$fromInt(elementHeight)),
						$elm$svg$Svg$Attributes$viewBox(
						'0 0 ' + A2($author$project$ViewPositions$createViewboxDimensions, $author$project$ViewVariables$viewportWidth, viewportHeight)),
						$elm$svg$Svg$Attributes$display('inline-block')
					]),
				A2(
					$elm$core$List$cons,
					A4($author$project$DrawToolbar$drawToolbar, program, mouseState, svgWindowWidth, svgWindowHeight),
					A4($author$project$DrawProgram$drawOnion, program, mouseState, svgWindowWidth, svgWindowHeight))));
	});
			...
			[
				...
				_List_fromArray(
					[
						...
						$rtfeldman$elm_css$Css$width(
						$rtfeldman$elm_css$Css$px(programWidth + $author$project$ViewVariables$scrollbarWidth)),
						$rtfeldman$elm_css$Css$height(
